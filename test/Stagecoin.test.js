...
describe("Stagecoin", function () {
  ...
  it("Minter can mint and balances update", async function () {
    await stage.connect(minter).mint(alice.address, ethers.utils.parseEther("100"));
  ...
  it("Transfer charges royalty and sends correct net", async function () {
    ...
    const fee = amount.mul(100).div(10000); // 1% of 10 = 0.1
  ...
  it("Burning works only with BURNER_ROLE", async function () {
    await stage.connect(minter).mint(alice.address, ethers.utils.parseEther("50"));
    // burner can burn alice's tokens (this implementation allows burning from arbitrary address by BURNER_ROLE)
    await stage.connect(owner).grantRole(await stage.BURNER_ROLE(), burner.address);
    await stage.connect(burner).burn(alice.address, ethers.utils.parseEther("10"));
    expect(await stage.balanceOf(alice.address)).to.equal(ethers.utils.parseEther("40"));
  });
  ...
  it("No royalty when recipient is address(0) or bp==0", async function () {
    ...
    await stage.connect(minter).mint(alice.address, ethers.utils.parseEther("10"));
    await stage.connect(alice).transfer(bob.address, ethers.utils.parseEther("1"));
    expect(await stage.balanceOf(royalty.address)).to.equal(0);
    expect(await stage.balanceOf(bob.address)).to.equal(ethers.utils.parseEther("1"));
  });
});
